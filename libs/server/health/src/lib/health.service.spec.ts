...
describe('HealthService', () => {
  ...
  describe('check', () => {
    it('returns ok when both database and redis are up', async () => {
    ...
    it('throws 503 when database is down, with full details in body', async () => {
      expect.assertions(4);
      ...
      try {
        ...
        expect(details['database']).toMatchObject({ status: 'down', error: 'bolt handshake failed' });
      }
    });

    it('includes redis status in 503 body when both are down', async () => {
      expect.assertions(2);
      mockGraphRepo.run.mockRejectedValue(new Error('db gone'));
      mockCache.ping.mockResolvedValue(false);
      ...
      try {
        ...
        expect(details['database']).toMatchObject({ status: 'down' });
        expect(details['redis']).toMatchObject({ status: 'down' });
      }
    });

    it('calls both database and redis checks on each invocation', async () => {
      mockGraphRepo.run.mockResolvedValue(undefined);
      mockCache.ping.mockResolvedValue(true);
